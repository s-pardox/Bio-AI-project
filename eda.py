...
"""
This module should be executed after the 5 runs for each of the 12 EA considered. Then repeat this file execution for 
...
def wandb_api(group: str):
    ...
    # Store the best test accuracy within the group; store the multiple DFs corresponding to different runs.
    best_test_acc = 0
    list_df_original = []
    for run in runs:
...
def plot_acc_div(alg_list: list, run_names: list, title: str):
    ...
    plt.savefig('report/{group}.png'.format(group=title))
    plt.show()
...
def comparison_plot(alg_tags: list, title: str):
    ...
    ax.set_xlabel('Algorithm Approach')
    ...
    plt.savefig('report/comparison_{group}.png'.format(group=title))
    plt.show()
...
def main(run_names: list()):
    ...
    Example of the input: 
        run_names = ['GA Test - Neutral Setting', 'ES-plus Test - Explorative Setting',
...
if __name__ == '__main__':
    # These hard-coded names refer to WandB group names, i.e. to effective runs that have been previously executed,
    # whose statistics have been recorded into WandB. You could have GAT architecture runs as well.
    list_name = ['CORA-GCN_GA-Neutral', 'CORA-GCN_GA-Explorative', 'CORA-GCN_DE-Exploitative', 'CORA-GCN_DE-Neutral',
                 'CORA-GCN_DE-Explorative', 'CORA-GCN_ES-comma', 'CORA-GCN_ES-plus', 'CORA-GCN_CMA-ES',
                 'CORA-GCN_PSO-Social', 'CORA-GCN_PSO', 'CORA-GCN_PSO-Cognitive']
    main(list_name)
